...
public class StickerListAdapter extends BaseAdapter {
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        ...
        if (Constants.selectedStickerCategory.equals(stickerModel.path)) {
        ...
        if (stickerModel.type == 0) {
            ...
            with.load(mContext.getFilesDir().getAbsolutePath()  + "/" + this.folder + "/" + stickerModel.path + ".png").intoImageView(viewHolder.icon);
        } else if (stickerModel.type == 1) {
            LoadBuilder<Builders.Any.B> with2 = Ion.with(this.mContext);
            with2.load(mContext.getFilesDir().getAbsolutePath()  + "/" + this.folder + "/" + stickerModel.path + ".png").intoImageView(viewHolder.icon);
